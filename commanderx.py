...
f="somefirmware2" # can be a user input
...
#r = redis.Redis(host = '192.168.2.66')     #this connects to rfsoc
#r = redis.Redis(host = '192.168.40.1')      #loops back to pc
#r = redis.Redis(host = '10.153.19.21') #loops back to mac??? maybe
r = redis.Redis(host = '10.153.63.255') #this keeps changing for some reason but his matches redis.conf
...
def usrInput():
    ...
    while True:
        ...
        #p.subscribe('python-channel') #for loopback on mac?? above is the actual one
        chnls_open = check_chnls()
